...
public class DAOContato extends DAOClass{
	...
	public void inserirContato(Contato contato) throws SQLException {
		...
		try {			
			...
			try {
				pstmt = conn.prepareStatement(querysql,PreparedStatement.RETURN_GENERATED_KEYS);
			...
			try {
				...
				if(tempaddr != null) {
					inserirEndereco(tempaddr,id_contato);
				}
			}catch(SQLException e) {
			...
			System.out.println("Inserção de contato bem sucedida!");
		}catch(SQLException e) {
			e.printStackTrace();
	...
	public void inserirEndereco(Endereco endereco, long idContato) throws SQLException {
		boolean abriuconn = false;
		//checar se tem uma conexão aberta
		try {
			if(conn.isClosed()) {
		...
		System.out.println("Inserção de endereço bem sucedida!");
	}
	...
	public void inserirTelefone(Telefone telefone, long id_contato) throws SQLException {
		...
		System.out.println("Inserção de telefone bem sucedida!");
	}	
